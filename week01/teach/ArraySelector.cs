public static class ArraySelector
{
    public static void Run()
    {
        ...
        var intResult = ListSelectorInt(l1, l2, select);
        Console.WriteLine("<int[]>{" + string.Join(", ", intResult) + "}"); // <int[]>{1, 2, 3, 2, 4, 4, 6, 8, 10, 5}

        var l3 = new[] { 'A', 'A', 'A', 'A', 'A'};
        var l4 = new[] { 'B', 'B', 'B', 'B', 'B'};
        select = [1, 2, 1, 2, 1, 2, 1, 2, 1, 2];
        var charResult = ListSelectorChar(l3, l4, select);
        Console.WriteLine("<char[]>{" + string.Join(", ", charResult) + "}"); // <char[]>{A, B, A, B, A, B, A, B, A, B}
    }

    private static int[] ListSelectorInt(int[] list1, int[] list2, int[] select)
    {
    ...
    private static char[] ListSelectorChar(char[] list1, char[] list2, int[] select)
    {
